...
public class StartGame extends JPanel {
    ...
    private static final int MOTORCYCLE_Y = 550; // 빨간 선의 Y좌표 (고정)

    public StartGame() {
        setPreferredSize(new Dimension(1280, 720)); // 창 크기 설정

        // TileManager를 생성하고 repaint()를 콜백으로 전달
        tileManager = new TileManager(1280, 720, this::repaint);
        ...
        clickMelody.setBounds(0, 0, 1280, 720);
        layeredPane.add(clickMelody, Integer.valueOf(0)); // 가장 뒤 레이어에 추가

        // 타일을 그리는 패널을 추가 (오토바이 이미지를 포함)
        ...
        tilePanel.setBounds(0, 0, 1280, 720);
        layeredPane.add(tilePanel, Integer.valueOf(2)); // 더 높은 레이어 값으로 설정

        // 마우스 움직임에 따라 오토바이 위치 업데이트
        this.addMouseMotionListener(new MouseAdapter() {
            @Override
            public void mouseMoved(MouseEvent e) {
                motorcycleX = e.getX() - 45; // 마우스 X좌표를 가져옴
                repaint();
            }
        });
